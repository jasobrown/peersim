...
/** 
 *  This config property define the parameters that defines separate
 *  experiments and the corresponding values. For each value, a
 *  different result file will be produced.
 */
...
/**
 *  This config property defines the number of repetitions that will
 *  be executed of the same experiment. 
 */
...
/** 
 * If set, it means the order of visiting each node is shuffled 
 * in each cycle 
 */
...
/**
 * This is the prefix for network observers. These have to be of
 * type {@link Observer}.
 */
...
throws IOException
{
	...
	if (args[0].equals("-d")) {
		...
		File resultdir = 
			new File((args.length == 3 ? args[2] : args[1])).getAbsoluteFile();
...
/**
 * Search in the configuration directory a configuration file to be
 * executed. If a configuration file can be executed concurrently,
 * search if any of its concurrent experiment has to be executed
 * yet.
 * 
 * @param configdir the directory containing configuration files to be
 *   executed
 * @param resultdir the directory where result files are written
 */
private static ConfigProperties 
	selectConfigFile(File configdir, File resultdir)
{
...
/**
 * Test a config file; read its content in order to verify whether it 
 * contains concurrent experiments. If so, it test whether any of them
 * has to be executed. If not, test if the entire file has to be
 * executed.
 * 
 * @param config the config file to be tested
 * @param resultdir the result directory
 * @return true if a file has been found, false otherwise
 */
private static ConfigProperties 
	checkFile(File config, File resultdir)
{
	...
	if (ranges.length == 0) {
		...
		dest = new File(resultdir, config.getName().substring(0, 
			config.getName().length()-4)+".txt");
		...
		while	(idx[0] < values[0].length && !isnew) {
			...
			// Compute the name of the destination file
			StringBuffer buffer = new StringBuffer(config.getName().substring(0, 
				config.getName().length()-4));
 ...
 * 
 * @param ranges the range specifications to be parsed
 * @param pars   the parameters that will change during the simulation
 * @param values the values that will be used for those parameters
 * @param name	 the name of the property describing ranges
 */
private static void parseRanges(String[] ranges, String pars[], 
	double[][] values, String name)
{
	for (int i=0; i < ranges.length; i++) {
		String[] array = Configuration.getString(ranges[i]).split(";");
		if (array.length != 2) {
			throw new IllegalParameterException(name+"."+i, 
				" should be formatted as <parameter>;<list>");
		}
		pars[i] = array[0];
 ...
 *  array. The index array is treated as a vector of digits; the first
 *  is managed  
...
public static void doExperiments(ConfigProperties properties)
{
	...
	if (Configuration.contains(PAR_EXPS) || ranges.length==0) {
		...
		values[ranges.length] = StringListParser.parseList("1:" + 
			Configuration.getInt(PAR_EXPS, 1));
	} else {
