...
/**
 * This class is an extension to the peersim standard graph factory
 * {@link peersim.graph.GraphFactory}. It arranges the graph edges
 * according to a preferential attachment approach called "HOT" model.
 * 
 * @author Gian Paolo Jesi
 */
public class InetFactory
{
 ...
 * @param pid
 *          Protocol index identifier; it is the parameter read from file
 * @param maxcoord
 *          Maximum edge size of the square. Default is 1.0 .
 * @param outdegree
 *          The out-degree each node must have.
 * @param alfa
 *          Clustering related parameter.
		...
		int outdegree, double alfa)
{
	...
	System.out.println(DEBUG_STRING + "size: " + size + " outdegree: "
			+ outdegree);

	// build outdegree roots
	System.out.println(DEBUG_STRING + "Generating " + outdegree
			+ " root(s), means out degree " + outdegree + "...");
...
//						.getProtocol(pid);
			...
			// System.out.println(DEBUG_STRING+"setting node "+i+" pointing to
	...
	for (int k = 0; k < how_many; k++) {
		...
		for (int j = 0; j < net_copy.size(); j++) {
			...
			double value = hops(parent, pid)
					+ (alfa * distance((Node) g.getNode(cur_node_index), parent, pid));
 ...
 * 
 * @param node
 *          the node to inspect to get its graph distance we are interested
 *          in
 * @param pid
